...
export default function AdminPageContent() {
  ...
  useEffect(() => {
    ...
    } else if (googleStatus === 'state_mismatch') {
      setError('Google Calendar connection failed validation. Try connecting again.');
    }
  }, [searchParams, user]);
                      ...
                      <div className="space-y-3">
                        <div className="flex flex-wrap items-center gap-2">
                          <h3 className="text-lg md:text-xl font-bold tracking-tight">
                            {booking.summary || booking.userName || 'Booking'}
                          </h3>
                          <span className={`px-2.5 py-1 rounded-lg text-xs font-medium uppercase tracking-wide ${booking.status === 'booked' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                            {booking.status || 'booked'}
                          </span>
                        </div>

                        <div className="grid gap-2 text-sm text-gray-700">
                          <p><span className="font-medium text-black">Artist:</span> {booking.userName || 'Unknown'}</p>
                          <p><span className="font-medium text-black">Email:</span> {booking.userEmail || 'Unknown'}</p>
                          <p><span className="font-medium text-black">Time:</span> {booking.start || 'Unknown'}</p>
                          {booking.end && <p><span className="font-medium text-black">Ends:</span> {booking.end}</p>}
                          {booking.timeZone && <p><span className="font-medium text-black">Time zone:</span> {booking.timeZone}</p>}
                          {booking.meetLink && (
                            <a href={booking.meetLink} target="_blank" rel="noreferrer" className="text-sm font-medium uppercase tracking-wide hover:opacity-60 transition-opacity">
                              Open Meet
                            </a>
                          )}
                          {booking.htmlLink && (
                            <a href={booking.htmlLink} target="_blank" rel="noreferrer" className="text-sm font-medium uppercase tracking-wide hover:opacity-60 transition-opacity">
                              Open Calendar Event
                            </a>
                          )}
                          {booking.cancelledBy && (
